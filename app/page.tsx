...
function blockFromPos(editor: ReturnType<typeof useEditor>, pos: number) {
  ...
  const blockPos = $pos.depth >= 1 ? $pos.before(1) : 0
...
export default function Home() {
  ...
  const handleGutterMove = (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    const found = elements.find((element) => {
      ...
      return y >= r.top && y <= r.bottom && r.left >= rect.left
    })
    if (!found) return
  ...
  const duplicate = (pos: number) => {
    ...
    editor.commands.focus(mapped.pos + mapped.node.nodeSize + 1)
  }
  ...
  return (
    <main className="app" style={styleVars}>
      <div className="editor-shell">
        ...
        <div className="editor-canvas" ref={editorRef} onMouseMove={handleGutterMove} onMouseLeave={() => setHoveredGutter(false)}>
          <div className="gutter" onMouseEnter={() => setHoveredGutter(true)}>
            {gutter && (
              <div className="gutter-hit" style={{ top: gutter.top, height: Math.max(gutter.height, 44) }} onMouseEnter={() => setHoveredGutter(true)} />
            )}
            {gutter && (hoveredGutter || window.matchMedia('(pointer: coarse)').matches) && (
              <div className="gutter-control" style={{ top: gutter.top + Math.max(0, (gutter.height - 42) / 2) }}>
      ...
      {menu && editor && (
        <div className="context-menu" style={{ left: menu.x, top: menu.y }} role="menu" aria-label="Nabídka bloku" onContextMenu={(event) => event.preventDefault()}>
      ...
      {searchOpen && (
        <div className="search-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setSearchOpen(false) }}>
          <section className="search-panel" role="dialog" aria-modal="true" aria-label="Hledat na webu">
            <div className="search-header"><Search size={18}/><input autoFocus value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') void search(); if (e.key === 'ArrowDown') setSelected((v) => Math.min(v + 1, Math.max(results.length - 1, 0))); if (e.key === 'ArrowUp') setSelected((v) => Math.max(v - 1, 0)); if ((e.metaKey || e.ctrlKey) && e.key === 'Enter' && results[selected]) { e.preventDefault(); insertResult(results[selected]) } }} placeholder="Hledat na webu…"/><button type="button" onClick={() => setSearchOpen(false)} aria-label="Zavřít"><X size={17}/></button></div>
